...
def compute_metrics(raw_simulation_log: SimulationLog) -> Metrics:
    ...
    if not raw_simulation_log:
        ...
        assert len(raw_simulation_log[0]) == 4, raw_simulation_log[0]

    simulation_log = [(task, _normalize_sumulation_status(status), score)
                      for task, status, score, _ in raw_simulation_log]
    simulation_log = [(task, _normalize_sumulation_status(status), score)
                      for task, status, score in simulation_log
                      if not status.is_invalid()]

    attempts = collections.defaultdict(int)
    pred_acc_per_task = collections.defaultdict(int)
    solved_at = {}
    first_solution_points = []
    for attempt_index, (task, status, score) in enumerate(simulation_log,
                                                          start=1):
        attempts[task] += 1
        if task not in solved_at and status.is_solved():
            first_solution_points.append(attempt_index)
            solved_at[task] = attempts[task]
        pred_acc_per_task[task] += (status.is_solved() == (score > 0.5))
    pred_acc = sum([pred_acc_per_task[task] * 1.0 / attempts[task] for
                    task in attempts.keys()])
...
class Evaluator():
    ...
    def __init__(self, task_ids: Tuple[str]):
        ...
        self._log_per_task_id: Dict[str, EvaluationLog] = {}
        self.attempts_per_task_index: List[int] = [0] * len(task_ids)

    def maybe_log_attempt(self,
                          task_index: int,
                          status: SimulationStatusLike,
                          score: float = 0.0,
                          action: Sequence = None) -> bool:
        ...
        Args:
            ...
            score: Normalized (typically via sigmoid) score of how likely the
                model thinks this action will solve the task.
                Added by rgirdhar, optional for previous parts of code.
            action: Actual action taken, to store in the results. Also added by
                rgirdhar.
        ...
        data_tuple = (task_id, status, score, action.tolist())
        self._log.append(data_tuple)
        if task_id not in self._log_per_task_id:
            self._log_per_task_id[task_id] = []
        self._log_per_task_id[task_id].append(data_tuple)
    ...
    def compute_all_metrics_per_task(self) -> Dict[str, Metrics]:
        """Computes metrics for each task separately.

        Returns:
            Dictionary of task to dictionary mapping metric name to computed
                value.
        """
        res = {}
        for task_id in self._task_ids:
            if task_id in self._log_per_task_id:
                res[task_id] = compute_metrics_normalized(
                    self._log_per_task_id[task_id], 1)
                res[task_id]['logs'] = self._log_per_task_id[task_id]
            else:
                res[task_id] = {}
                res[task_id]['logs'] = []
        return res
