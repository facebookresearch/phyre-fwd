...
def create_balanced_eval_set(cache: phyre.SimulationCache, task_ids: TaskIds,
    ...
    if len(positive_indices) > half_size:
        positive_indices = rng.choice(positive_indices,
                                      half_size,
                                      replace=False)
    negative_indices = rng.choice(negative_indices,
...
def finetune(
    ...
    if len(set(task_indices)) == 1:
        observations = np.expand_dims(
            simulator.initial_scenes[task_indices[0]], 0)
    else:
...
def train(output_dir,
    ...
    for batch_id, batch_indices in enumerate(train_indices_sampler(),
        ...
        batch_is_solved = is_solved[batch_indices].to(device,
                                                      non_blocking=True)
        if roll_fwd_ratio > 0:
            batch_observations = np.array(
                roll_fwd_obs(batch_task_indices,
                             batch_actions,
                             simulator,
                             roll_fwd_ratio,
                             nframes=1))
            batch_observations = torch.LongTensor(
                np.squeeze(batch_observations, axis=1))
        ...
        for batch_start in range(0, len(actions), batch_size):
            ...
            if roll_fwd_ratio > 0:
                observations = np.array(
                    roll_fwd_obs([task_id] * len(observations),
                                 batch_actions,
                                 simulator,
                                 roll_fwd_ratio,
                                 nframes=1))
                observations = torch.LongTensor(
                    np.squeeze(batch_observations, axis=1))
            batch_scores = model(None,
                                 batch_actions,
                                 preprocessed=preprocessed)
            # Since the model is trained with BCELoss, normalize using sigmoid
            batch_scores = torch.nn.Sigmoid()(batch_scores)
